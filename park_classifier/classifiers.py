...
from keras.layers import Conv2D, BatchNormalization, Dense, Activation, Flatten, MaxPooling2D, Dropout, SpatialDropout2D, Input, AveragePooling2D
...
class MyNet1(Classifier):
    def build(self, input_shape=(W, H, 3), num_classes=2):
        ...
        model.compile(optimizer=adam, metrics=['accuracy', 'binary_accuracy'], loss='binary_crossentropy')
        return model
...
class CustomAlex1(Classifier):
    def build(self, input_shape=(W, H, 3), num_classes=2) -> Sequential:
        model = Sequential()
        model.add(Conv2D(16, (3, 3), input_shape=input_shape, strides=(4, 4), padding='same'))
        model.add(Activation('relu'))
        model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2)))

        model.add(Conv2D(20, (3, 3), strides=(1, 1), padding='same'))
        model.add(Activation('relu'))
        model.add(MaxPooling2D(pool_size=(3, 3), strides=(2, 2)))
