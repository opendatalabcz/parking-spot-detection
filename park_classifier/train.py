...
from classifiers import CustomAlex1, CVGG16, CResNet50, CVGG19, CInceptionV3, CDenseNet121, CXception, MyNet2, MyNet1, \
    MyNet3, MyNet4, MyNet5, CMobileNet, CMobileNetV2, CustomAlex2, CNasNetMobile
...
def load_images(paths):
    imgs = []
    for path in paths:
        imgs.append(np.asarray(load_img(path, target_size=(W, H))))
        # imgs.append(np.asarray(load_img(path, target_size=(W, H), interpolation='bicubic')))
        # imgs.append(ParkModel.reshaped_image(cv2.imread(path), W, H))
        # imgs.append(resize(imread(path), (W,H)))
    return imgs
...
def load_pk_labels(path):
    for path, subdirs, files in os.walk(path):
        ...
        for name in files:
            ...
            if "occupied" in img_path.lower() and "empty" in img_path.lower():
                print(img_path)
...
class Generator(Sequence):
    ...
    def __getitem__(self, index):
        ...
        if self.generate_mutations:
            ...
            mut_x, mut_y = self.image_gen.flow(train_split, val_split,
                                               int(self.batch_size * self.mutation_split)).next()
...
if __name__ == "__main__":
    ...
    mc = ModelCheckpoint(
        'net5-{epoch:02d}_accuracy-{accuracy:.4f}_val-loss-{val_loss:.4f}_val-acc-{val_accuracy:.4f}.h5',
        monitor='val_accuracy', mode='max', save_best_only=True, verbose=1, period=1)
    ...
    model.fit_generator(train_gen, validation_data=val_gen, callbacks=[tensorboard, mc, lr], shuffle=True, verbose=1,
                        steps_per_epoch=len(cnr_labels)/200,
                        workers=3,
                        validation_steps=len(cnr_labels)/200,
                        epochs=200
                        )
